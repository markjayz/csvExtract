...
workbook = pd.read_csv('D:/Work/Files/QuadX/Templates/sample_csv.csv')
today = date.today()
with open(f'{today}_created-address.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    ...
    def my_function():
        for index, row in workbook.iterrows():
                url = "https://api.staging.quadx.xyz/v1/locations/provinces/"
                headers = {
                    ...
                    if province_name.lower() == row['state'].lower():
                        ...
                        for city in data_city['data']:
                            ...
                            if row['city'].lower() in city_name.lower():
                                city_id = city['id']
                                url_district = 'https://api.staging.quadx.xyz/v1/locations/cities/' + str(city_id) + '/districts'
                                ...
                                for district in data_district['data']:
                                    if (not pd.isna(row['line_2'])):
                                        if row['line_2'] != '.' and district['name'].lower() in row['line_2'].lower():
                                            ...
                                            update_url = "https://sc-api.staging.shippingcart.com/api/addresses/" + str(row['id']) + "/update"
                                            update_headers = {
                                                    'Content-Type': 'application/json',
